...
const [searchField, setSearchField] = useState<"todos" | "nombre" | "telefono" | "email">("todos");
  ...
  const handleAddInteraccion = async () => {
    ...
    try {
      ...
      if (customerId) {
        ...
        // intentar asegurar token demo si no existe
        await client.ensureAuth();
        await client.rpc("create_customer_interaction", payload);
...
const getPageItems = () => {
  ...
  // Siempre mostrar primera, última y alrededores de la actual
...
return (
      ...
      <CardContent>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-4">
          ...
          <div className="flex-1 flex gap-2">
            ...
            {/* Campo de búsqueda (opcional si mantienes filtro local) */}
  ...
  {/* Info pequeña de resultados */}
  <span className="text-xs text-muted-foreground">
    Mostrando {(page - 1) * pageSize + 1}–
    {Math.min(page * pageSize, totalCount)} de {totalCount} clientes
  </span>
  ...
  <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-4">
    ...
    <div className="flex items-center gap-1 rounded-full border bg-card px-2 py-1 shadow-sm">
      {/* Flecha izquierda (solo si se puede ir atrás) */}
      {canGoPrev && (
        <Button
          variant="ghost"
          size="icon"
...
<Button
  ...
  className={
    item === page
      ? "h-7 min-w-[2rem] rounded-full bg-gray-100/60 text-gray-900 shadow-sm hover:bg-primary/20 transition-colors"
      : "h-7 min-w-[2rem] rounded-full hover:bg-gray-100/40 transition-colors"
  }
...
>
  {item}
      ...
      {/* Flecha derecha (solo si se puede ir adelante) */}
      {canGoNext && (
        <Button
          variant="ghost"
          size="icon"
              ...
              <TabsContent value="resumen" className="space-y-4">
                ...
                <TabsContent value="compras" className="space-y-4">
                  {dataStore.ventas
                    ...
                    .map((venta) => {
                      const vehiculo = dataStore.vehiculos.find((vh) => vh.id === venta.vehiculoId);
                      return (
                        <Card key={venta.id}>
                          <CardHeader>
                            <div className="flex items-start justify-between">
                              ...
                              <div className="text-right">
                                <div className="text-lg font-bold">{formatCurrency(venta.precioFinal)}</div>
                  ...
                  {dataStore.ventas.filter((v) => v.clienteId === selectedCliente.id).length === 0 && (
